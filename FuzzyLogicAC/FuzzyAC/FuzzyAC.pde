...
public void createFuzzyEngine(){
    // TODO: Create the Fuzzy Logic objects
    engine = new Engine();
    engine.setName("Fuzzy Logic AC_CA1");
    ...
    inputVariable1.addTerm(new Triangle("hot", 20, 28, 32));
    ...
    //commented out are needed sometimes
    ...
    outputVariable.addTerm(new Trapezoid("cold", -10, -10, -6, 0)); 
    outputVariable.addTerm(new Triangle("noChangeNeeded", -1, 0, 1));
    outputVariable.addTerm(new Trapezoid("hot", 0, 6, 10, 10)); 
    ...
    ruleBlock.addRule(Rule.parse("if (target is verycold) and (room is not verycold) then command is cold", engine)); 
    ruleBlock.addRule(Rule.parse("if (target is veryhot) and (room is not veryhot) then command is hot", engine)); 
    
    ruleBlock.addRule(Rule.parse("if (room is warm or room is hot or room is veryhot) and (target is cold) then command is cold", engine));
    ruleBlock.addRule(Rule.parse("if (room is verycold) and (target is cold) then command is hot", engine)); 
    ruleBlock.addRule(Rule.parse("if (room is cold) and (target is cold) then command is noChangeNeeded ", engine));
    
    ruleBlock.addRule(Rule.parse("if (target is warm) and (room is hot) then command is cold", engine)); 
    ruleBlock.addRule(Rule.parse("if (target is warm) and (room is cold) then command is hot", engine)); 
    ruleBlock.addRule(Rule.parse("if (target is warm) and (room is warm) then command is noChangeNeeded", engine));
    
    ruleBlock.addRule(Rule.parse("if (room is cold or room is warm) and (target is hot) then command is hot", engine)); 
    ruleBlock.addRule(Rule.parse("if (target is hot) and (room is hot) then command is noChangeNeeded", engine));
    ruleBlock.addRule(Rule.parse("if (target is hot) and (room is veryhot) then command is cold", engine)); 
    
    engine.addRuleBlock(ruleBlock);
   
}//close createFuzzyEngine

private float fuzzyACEvaluate(float rt, float tt){
    
    /* TODO: uncomment to execute the Fuzzy Logic engine
